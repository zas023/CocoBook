...
import com.thmub.cocobook.utils.NotificationUtils;
...
public class DownloadService extends BaseService {
    ...
    public void onCreate() {
        super.onCreate();
        mNotificationUtils = new NotificationUtils(getApplication());
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        //是否删除数据的问题
        Disposable deleteDisp = RxBusManager.getInstance()
                ...
                .subscribe(
                        (event) -> {
                            ...
                            //返回状态
                            RxBusManager.getInstance().post(new DeleteBookEvent(isDelete, event.collBook));
                        }
                );
    ...
    private void executeTask(DownloadTaskBean taskEvent) {
        mNotificationUtils.showNotification(mDownloadTaskList.indexOf(taskEvent));
        Runnable runnable = () -> {
    ...
    private void postDownloadChange(DownloadTaskBean task, int status, String msg) {
        if (mDownloadListener != null) {
            ...
            if (status==DownloadTaskBean.STATUS_LOADING){
                mNotificationUtils.updateProgress(position,Integer.valueOf(msg),task.getLastChapter());
            }
            //通过handler,切换回主线程
            mHandler.post(
